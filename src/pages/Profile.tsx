...
export function Profile() {
  ...
  const fetchFriendshipStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const isFriend = data.friends.some((f: any) => f.id === profileId);
        if (isFriend) {
        ...
        const sentRequest = data.requests.sent.find((r: any) => r.id === profileId);
        if (sentRequest) {
        ...
        const receivedRequest = data.requests.received.find((r: any) => r.id === profileId);
        if (receivedRequest) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // 게임 히스토리 가져오기
        const gamesRes = await fetch(`/api/games?userId=${profileId}&limit=10`, {
          credentials: 'include',
        });
  ...
  const handleAcceptFriendRequest = async () => {
    ...
    try {
      setActionLoading(true);
      const response = await fetch(`/api/friends/${profileId}/accept`, {
        method: 'PUT',
        credentials: 'include',
      });
