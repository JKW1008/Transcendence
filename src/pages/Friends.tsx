...
export function Friends() {
  ...
  const fetchFriends = async () => {
    try {
      ...
      setSentRequests(data.requests.sent);
      setReceivedRequests(data.requests.received);
    } catch (err) {
  ...
  const removeFriend = async (userId: string, action: string) => {
    try {
      setError('');
                  ...
                  {receivedRequests.map((request) => (
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => removeFriend(request.id, 'Reject request')}
                ...
                <div className="space-y-3">
                  {friends.map((friend) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => removeFriend(friend.id, 'Remove friend')}
                        className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors text-sm"
                      >
                        Remove
                      </button>
                ...
                <div className="space-y-3">
                  {sentRequests.map((request) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => removeFriend(request.id, 'Cancel request')}
                      ...
                      >
                        ...
                        {result.friendshipStatus === 'pending' && result.requestType === 'received' && (
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => removeFriend(result.id, 'Reject')}
                              className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors text-sm"
                            >
                              Reject
                            </button>
